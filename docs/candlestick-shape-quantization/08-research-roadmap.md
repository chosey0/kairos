...
```text
A안(기본): winsorized logit + boundary flag 4bit (open==low, open==high, close==low, close==high)
B안(비교): 기존대로 제외
...
```text
Stage D1. 단일 지수
  KOSPI(KS11), KOSDAQ(KQ11), NASDAQ(IXIC), S&P500(GSPC), DJIA(DJI) 중 1개 선택
  목적: 파이프라인 검증, feature/tokenizer 설계 확정, 초기 K sweep

Stage D2. 국가별 주요 지수
  미국(IXIC, GSPC, DJI) 또는 대한민국(KS11, KQ11)
  목적: 같은 시장 내 cross-index token 일반화 확인 (H1의 1차 검증)

Stage D3. 세계 주요 지수
  IXIC, SPX, DJI, RUT, KS11, KQ11, N225, NDX, SOX, GSPC, TWII
...
작업:
- index universe 확정. VIX는 거래 대상이 아니고 shape 의미가 다르므로 main set에서 제외하고 별도 분석 set으로 둔다.
...
- K sweep: 8, 12, 16, 24, 32 (계층형은 coarse 8 × fine 8 형태로 등가 용량 비교)
