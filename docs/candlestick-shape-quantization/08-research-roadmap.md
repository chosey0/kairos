...
`7/17/37`만 실행했다. 결과는
[results/phase-01-step-03-vq-final-gate.md](results/phase-01-step-03-vq-final-gate.md)에 기록했다
(run artifacts: `notebooks/runs/.../step-03-vq-tokenizer/all-datasets-multi/cfg-d59bafed/`, 로컬 전용). `vqvae_latent_kmeans`, `fsq`, `bsq`는 reconstruction, seed stability, token usage에서
...
진행 상태(2026-07-07 완료): Phase 1 final gate에서 확정한 `kmeans_boundary_aware K=32`
tokenizer를 고정하고, 8개 dataset(D1 daily 4개, D1 KR 1m 2개, D2 KR daily/1m 2개)에 대해
token sequence corpus와 label store를 생성한다. run 레이아웃은
`notebooks/runs/candlestick-shape-quantization/phase-02-token-corpus/step-01-corpus-and-labels/<dataset>/cfg-<hash>/run-<ts>_seed-<n>/`
로 둔다. seed `7`을 canonical corpus로 사용하고, seeds `7/17/37`의 token 통계 안정성을 함께 보고한다.
결과 해석은 [results/phase-02-step-01-token-corpus.md](results/phase-02-step-01-token-corpus.md)에 기록했다. 모든 dataset의 OHLC join coverage는 100%였다.
...
- Williams Fractal pivot column은 Phase 3 segmentation 준비용으로만 저장한다. pivot label은 미래 n=2개
  캔들이 있어야 확정되므로, downstream feature로 사용할 수 있는 anchor는 pivot timestamp가 아니라
  `fractal_confirmed_at` 이후다.
...
판정(2026-07-07): 순차 구조는 존재하지만 vocab별로 강도가 다르다. 1m full/boundary vocab과 KR daily full vocab은
surrogate shuffle 대비 information gain이 높다. 반면 daily interior-only 구조는 dataset별로 불안정하다.
Phase 3는 진행하되 full/interior/boundary decomposition과 Markov-1 surrogate test를 유지하고,
interior-only motif의 기대치는 낮춘다.
Daily-only 재집계([results/phase-02-step-01-daily-only-gate.md](results/phase-02-step-01-daily-only-gate.md))에서도
full vocab은 KR daily 중심으로만 강했고, interior-only는 weighted aggregate 기준 surrogate보다 낮아
daily interior motif는 ablation으로만 둔다.

## Phase 3. 다중 Candle Motif Vocabulary

목표: token sequence에서 variable-length motif vocabulary를 학습한다.

Phase 3 착수 전 미니스텝 판정(2026-07-07):
1m track의 약하지만 유의한 순차 구조가 persistence(run), intraday time-bucket composition,
또는 Markov-1 이후의 고차 구조 중 무엇으로 설명되는지
기존 corpus CSV만으로 진단했다([results/phase-02-step-01-1m-structure-diagnosis.md](results/phase-02-step-01-1m-structure-diagnosis.md)). 결론은 단순 run이나 시간대 조성 차이만으로 소진되지 않는다.
RLE 후 full-vocab IG가 surrogate 대비 q=1.00으로 더 강하게 남고, bucket-preserving surrogate에서도
관측 IG가 남으며, 2차 IG도 Markov-1 surrogate 대비 q=1.00이다. 따라서 Phase 3는 raw token BPE만
단독 적용하지 않고 RLE ablation, time-bucket stratification, Markov-1 surrogate 검정을 필수 guardrail로 둔다.
Phase 2 interior-only 구성은 실제 인접쌍만 세고 있어 boundary 제거로 인공 adjacency를 만들지 않는 것으로 확인했다.
...
경로 B 보조 후보: Williams Fractal leg segmentation(H->L / L->H 단위 sequence). Phase 2 corpus의
`fractal_high`, `fractal_low`, `fractal_confirmed_at` 컬럼을 사용하되, 예측 anchor는 반드시
`fractal_confirmed_at` 이후로 둔다.
